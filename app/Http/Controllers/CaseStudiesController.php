...
class CaseStudiesController extends Controller
{
    public function index(Request $request){
        $data = ModulesData::where('module_id', 11)->where('status','active');
        ...
        $arr['recent_data'] = ModulesData::where('module_id', 11)->where('status','active')->orderBy('id','desc')->take(3)->get(); 
        $arr['archives'] = $this->lastThreeMonths();
        return view('case_studies.index')->with($arr);
    }   
    ...
    public function detail($slug){
    	...
    	$data['recent_data'] = ModulesData::where('module_id', 11)->where('status','active')->orderBy('id','desc')->take(3)->get(); 
    	
		$data['archives'] = $this->lastThreeMonths();
    	 
    	return view('case_studies.detail')->with($data); 		
    }
